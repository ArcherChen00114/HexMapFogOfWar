...
public class HexMesh : MonoBehaviour {
	...
	//判断这些区域能否修改颜色和响应碰撞（事件）
	//以此来区分水体以及地面。

	[NonSerialized] List<Vector2> uvs, uv2s;//保存uv，但是哪里调用呢？	
	...
	public void Apply()
	{
		...
		if (useUVCoordinates){
			hexMesh.SetUVs(0, uvs);//调用了HexMesh的底层算法，设定多个点的数值，表示图形的几个点所代表的UV值大小
			//(0,1,0,1)左右上下表示UV分布，主要用于河流来控制Shader的贴图方向
